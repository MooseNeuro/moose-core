...
input_x = moose.StimulusTable('/model/xtab')
input_x.vector = xarr
...
input_y = moose.StimulusTable('/model/ytab')
input_y.vector = yarr
...
plt.subplot(3,1,1)
plt.title('z = {}'.format(function.expr))
plt.plot(x_rec.vector, result.vector, 'r-', label='moose')
z = function.c['c0'] * np.exp(function.c['c1'] * xarr) * np.cos(yarr) + np.sin(np.arange(len(xarr)) * dt)
plt.plot(xarr, z, 'b--', label='numpy')
plt.xlabel('x')
plt.ylabel('z')
plt.legend()
