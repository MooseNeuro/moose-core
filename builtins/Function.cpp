...
const Cinfo *Function::initCinfo()
{
    ...
    static ValueFinfo<Function, bool> allowUnknownVariable(
        ...
        "Defaults to *true*. \n",
        &Function::setAllowUnknownVariable, &Function::getAllowUnknowVariable);

    static ElementValueFinfo<Function, string> expr(
        "expr",
        "Mathematical expression defining the function. The underlying parser\n"
        "is exprtk (https://archive.codeplex.com/?p=exprtk) . In addition to "
        "the\n"
        "available functions and operators  from exprtk, a few functions are "
        "added.\n"
        "\nMajor Functions\n"
        ...
        "rnd         0       rand(), random float between 0 and 1, honors "
        "global moose.seed.\n"
    ...
    static FieldElementFinfo<Function, Variable> inputs(
        ...
        " entry.",
        Variable::initCinfo(), &Function::getX, &Function::setNumVar,
        &Function::getNumVar);
    ...
    static LookupValueFinfo<Function, string, unsigned int> xindex(
        "xindex",
        "Returns the index of a given variable which can be used with field "
        "`x`.\n"
        "Note that we have a mechanism to map string (variable name) to "
        "integer (variable index).",
        &Function::setVarIndex, &Function::getVarIndex);
    ...
    static DestFinfo setSolver(
        "setSolver", "Assigns solver to this Function.",
    ...
    static Finfo *functionFinfos[] = {
        &value,          &evalResult,  &rate,           &derivative,
        &mode,           &useTrigger,  &doEvalAtReinit, &allowUnknownVariable,
        &expr,           &numVars,     &inputs,         &xindex,
        &constants,      &independent,
        &setSolver,  // DestFinfo
        &proc,           requestOut(), valueOut(),      rateOut(),
        derivativeOut(),
    };

    static string doc[] = {"Name",        "Function",
...
void Function::setNumVar(const unsigned int num)
{
    ...
    // from the expression.
    cerr << "Function::setNumVar is deprecated. Function object infers number "
            "of variables from the expression."
         << endl;
}
...
void Function::reinit(const Eref &e, ProcPtr p)
{
    if (!(valid_ || parser_->GetExpr().empty())) {
        MOOSE_WARN("Error: " << e.objId().path()
                             << "::reinit() - invalid parser state" << endl
                             << " Expr: '" << parser_->GetExpr() << "'.");
        return;
    }
...
void Function::setSolver(const Eref &e, ObjId newStoich)
{
    ...
    if (newStoich.bad()) {
        cout << "Warning: Function::setSolver: Bad Stoich " << e.id().path()
             << endl;
        return;
    }
    ...
    if (!newStoich.element()->cinfo()->isA("Stoich")) {
        cout << "Warning: Function::setSolver: object " << newStoich.path()
             << "is not a Stoich for " << e.id().path() << endl;
        return;
    }
